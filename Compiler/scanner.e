...
feature {Any}
	...
	do
		...
		when 'a' then -- "abstract" - 8, "alias", "as"
			...
			when 8 then
				if buffer.is_equal (keywords.item (abstract_token)) then
					Result := abstract_token
				else
					Result := register_buffer_and_return_identifier_token
				end
			else
		...
		when 'c' then -- "case", "cluster", "concurrent", "const"
			...
			when 7 then
				if systemMode and then buffer.is_equal (keywords.item (cluster_token)) then
		...
		when 'e' then -- "else" 4, "elsif" -5, "end" - 3, "ensure" 6, "entry" - 5, "extend" - 6, "elseif" - 6
			inspect
				buff_len
			when 3 then -- end
			...
			when 5 then -- elsif
				if buffer.is_equal (keywords.item (elsif_token)) then
					Result := elsif_token
				elseif systemMode and then buffer.is_equal (keywords.item (entry_token)) then
					Result := entry_token
				else
					Result := register_buffer_and_return_identifier_token
				end
		...
		when 'f' then -- "final" - 5, "from" - 4, "foreign" - 7
			inspect
				buff_len
			when 4 then
				if systemMode and then buffer.is_equal (keywords.item (from_token)) then
					Result := from_token
				else
					Result := register_buffer_and_return_identifier_token
				end -- if
		...
		when 'h' then
			if systemMode and then buffer.is_equal (keywords.item (hide_token)) then
				Result := hide_token
			else
				Result := register_buffer_and_return_identifier_token
			end -- if		
		...
		when 'r' then -- "raise" - 5, "ref" - 3, "rename" - 6, "require" - 7, "return" - 6, "rigid" - 5, "rtn" - 3
			...
			when 6 then -- return, rename
				if buffer.is_equal (keywords.item (return_token)) then
					Result := return_token
				elseif systemMode and then buffer.is_equal (keywords.item (rename_token)) then
					Result := rename_token
				else
					Result := register_buffer_and_return_identifier_token
				end -- if
