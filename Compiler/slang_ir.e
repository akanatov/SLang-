...
-- [init Identifier]
-- [use {(Identifier| StringConstant) [ì:î Options end]} end]
...
--7 [pure|safe] Identifier [FormalGenerics] [Parameters] [ì:î Type] [EnclosedUseDirective]        
--       ([RequireBlock] InnerBlockDescriptor|foreign [EnsureBlock] [end] ) | (ì=>îExpression )
-- Parameters    : ì(îParameterDescriptor {î;î ParameterDescriptor}ì)î 
inherit
	Comparable
		redefine	
			out, is_equal
...
feature {Any}
	...
	do
		...
		-- clear inner block or ì=>îExpression
...
-- { ( MemberVisibility ì:î {MemberDeclaration}) |  MemberDeclaration }
-- [InvariantBlock]
-- end
inherit
	Comparable
...
feature {Any}
	...
	formalGenerics: Array [FormalGenericDescriptor]
		--  ì[î FormalGenericDescriptor {ì,î FormalGenericDescriptor}ì]î 
...
--13 Identifier ([ìextendî Type ] [ìinitî [Signature]])| [ì:î (UnitTypeDescriptor | RoutineType)]
inherit
	Comparable
		undefine
			out
...
-- Identifier [ìextendî Type ] [ìinitî [Signature]]
inherit
...
class AttachedUnitAttributeDescriptor
	...
	-- |
	-- ( [const|rigid] Identifier [ì:î AttachedType] is ConstantExpression) 
	-- |
	-- (Identifier ì:î Type rtn ì:=î [[ Parameters] HyperBlock ] )
inherit
	UnitAttrDescriptor
		redefine
			out
...
-- rtn ì:=î[ [Parameters] HyperBlockDescriptor ]
inherit
	Any
		undefine
			out
...
create	
	init
feature {Any}
	-- ì(îParameterDescriptor {î;î ParameterDescriptor}ì)î	
	parameters: Array [ParameterDescriptor]
...
-- RangeExpression : Expression [ì{îOperatorName ConstantExpression "}"] ì..îExpression
inherit
	ExpressionDescriptor
	end
create
...
-- ì(î[TupleElement {ì,î TupleElement}]ì)î
inherit
	ExpressionDescriptor
	end
create
...
-- Expression is ì?î| UnitTypeDescriptor
inherit
	ExpressionDescriptor
	end
create
...
-- [UnitTypeDescriptor ì.î] CharacterConstant |IntegerConstant |RealConstant |StringConstant |Identifier  
inherit
	ConstObjectDescriptor
		undefine
			is_equal, infix "<"
...
--  new UnitType [ì.îinit] [ Arguments ]
inherit
	ExpressionDescriptor
	end
	RoutineArguments
...
--39 Type: [î?î] AttachedTypeDescriptor
...
--41 ì(î[TypeDescriptor {ì,î TypeDescriptor}]ì)î[ì:î TypeDescriptor]
-- (ì(î[Type {ì,î Type}]ì)î[ì:î Type])| (ì:î Type)
inherit
	Comparable
		redefine
			out, is_equal
...
--44 UnitTypeDescriptor {ì|î UnitTypeDescriptor} 
inherit
	AttachedTypeDescriptor
	end
create
...
-- ì(î[TupleFieldDescriptor {ì,î|î;î TupleFieldDescriptor}]ì)î
inherit
	AttachedTypeDescriptor
	end
create
...
-- [Identifier {ì,î Identifier}ì:î] UnitTypeDescriptor
inherit
	Comparable
		undefine
			out, is_equal
...
-- GenericInstantiation: ì[îType {ì,î Type}ì]î 
inherit
	UnitTypeCommonDescriptor
		rename
