...
-- [RequireBlock]
-- (InnerBlock [EnsureBlock] BlockEnd)|(((â€œ=>â€Expression)|foreign) [EnsureBlock BlockEnd])
inherit
	RoutineDescriptor
		redefine
			is_equal
...
-- [RequireBlock]
-- ((InnerBlock) [EnsureBlock] BlockEnd) | ((abstract|foreign|none|old| (â€œ=>â€Expression))[EnsureBlock BlockEnd])
inherit
...
class InitDeclarationDescriptor
    ...
    -- [â€œ:â€ (init  [Arguments]) | (ParentInitCall {â€œ,â€ ParentInitCall} [â€œ,â€ init  [Arguments]])]
...
feature {Any}
	...
	local
	do
		-- check that propoer constructoÐº is in place 
...
-- if Expression (â€œisâ€ Alternatives)|(BlockStart StatementsList)
-- {elsif Expression (â€œisâ€ Alternatives)|(BlockStart StatementsList)}
-- [else StatementsList]
-- BlockEnd
--
-- Alternatives:
-- â€œ:â€ AlternativeTags StatementsList
-- {â€œ:â€ AlternativeTags StatementsList}
...
-- Expression [[GroupStart OperatorName ConstantExpression GroupEnd] â€œ..â€Expression ] | UnitType
inherit
	SmartComparable
		undefine
			out
